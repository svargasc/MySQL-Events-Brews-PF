    ...
    <>
      <div className="h-screen overflow-auto bg-[url('https://i.ibb.co/LQf91TG/fondo-EB.webp')] bg-cover bg-center flex flex-col justify-center items-center">
        ...
        <div className='w-full flex h-full justify-center items-center gap-3 flex-wrap'>
          {
            events.map(event => (
              <div key={event.id} className='bg-[#000000a4] w-1/4 h-2/3 flex flex-col justify-between rounded-2xl text-[#FFEEB3]' >
                ...
                <p className='text-center text-xl'>{dayjs(event.date).utc().format("YYYY/MM/DD")}</p>
                <div className='flex justify-between items-center'>
                  <Link className='flex items-center justify-center bg-[#FFEEB3] text-[#AC703E] text-xl m-2 font-bold h-12 w-2/5 rounded-full mt-2 hover:bg-[#AC703E] hover:text-[#FFEEB3] duration-300' to={`/events/${event.id}`}>Editar evento</Link>
                  <button className='bg-[#FFEEB3] text-[#AC703E] text-xl m-2 font-bold h-12 w-1/3 rounded-full mt-2 hover:bg-red-700 hover:text-[#FFEEB3] duration-300' onClick={()=>{deleteEvent(event.id)}}>Eliminar</button>
